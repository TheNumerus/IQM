...
void main() {
    ...
    int halfSize = int(push_consts.kernelSize / 2);
    ...
    for (int i = 0; i < push_consts.inChannels; i++) {
        int inChannelOffset = i * int(push_consts.kernelSize * push_consts.kernelSize * gl_NumWorkGroups.z);

        for (int y = -halfSize; y <= halfSize; y++) {
            int srcY = srcCenterY - y;
            for (int x = -halfSize; x <= halfSize; x++) {
                int posOffset = (int(push_consts.kernelSize) * (y + halfSize) + (x + halfSize)) * int(gl_NumWorkGroups.z);
