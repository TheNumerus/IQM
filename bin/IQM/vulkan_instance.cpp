...
void IQM::Bin::VulkanInstance::waitForFence(const vk::raii::Device &device, const vk::raii::Fence &fence) {
    auto res = device.waitForFences({fence}, true, std::numeric_limits<uint64_t>::max());
...
void IQM::Bin::VulkanInstance::initQueues() {
    ...
    this->physicalDevice = physicalDevice.value();
    ...
    this->queue = std::make_shared<vk::raii::Queue>(this->device.getQueue(this->queueFamilyIndex, 0));

    if (dedicatedTransferQueue) {
        this->transferQueue = std::make_shared<vk::raii::Queue>(this->device.getQueue(this->transferQueueFamilyIndex, 0));
    } else {
        this->transferQueue = this->queue;
    }
    ...
    this->commandPool = std::make_shared<vk::raii::CommandPool>(vk::raii::CommandPool{this->device, commandPoolCreateInfo});
    ...
    auto bufs = vk::raii::CommandBuffers{this->device, commandBufferAllocateInfo};
    ...
    if (dedicatedTransferQueue) {
        ...
        this->commandPoolTransfer = std::make_shared<vk::raii::CommandPool>(vk::raii::CommandPool{this->device, commandPoolCreateInfoTransfer});
        ...
        this->cmd_bufferTransfer = std::make_shared<vk::raii::CommandBuffer>(std::move(vk::raii::CommandBuffers{this->device, commandBufferAllocateInfo}.front()));
    }
}
