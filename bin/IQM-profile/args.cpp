...
IQM::ProfileArgs::ProfileArgs(const unsigned argc, const char *argv[]) {
    ...
    while (i < argc)  {
        ...
        if (strcmp(argv[i], "--ref") == 0) {
            if (i + 1 >= argc) {
                throw std::invalid_argument("Missing reference argument");
            }
        ...
        if (i + 1 >= argc) {
            throw std::invalid_argument("Missing value argument");
        }
